...
class CopydetectConfig:
    ...
    window_size_comment: int = field(init=False, default=0)
    comment_mode: str = "kgram"     # "kgram" | "cosine"
    comment_ngram_n: int = 3        # cosine용 n-gram 크기
    comment_weight: float = 1.0# 최종 합산 시 주석 가중치
    # --- Notebook preview ---
    html_notebook_preview: bool = False
    notebook_preview_dir: str = "notebook_preview"
    # === [ADD] ipynb 텍스트 검사 옵션 ===
    include_markdown: bool = False     # ipynb의 markdown 셀도 검사
    ...
    def __post_init__(self):
        ...
        self.window_size_comment = self.comment_guarantee_t - self.comment_noise_t + 1
